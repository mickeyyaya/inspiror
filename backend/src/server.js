...
exports.app.post("/api/generate", async (req, res) => {
    ...
    if (req.body.currentBlocks !== undefined &&
    ...
    console.log(`[API] Generation request received | Messages: ${req.body.messages.length} | Blocks length: ${req.body.currentBlocks?.length || 0}`);
    try {
        const language = VALID_LANGUAGES.has(req.body.language)
            ? req.body.language
            : "en-US";
        const result = await llmService_1.llmService.generateStream(req.body.messages, req.body.currentBlocks, language);
...
exports.app.post("/api/convert-to-blocks", async (req, res) => {
    ...
    if (!VALID_LANGUAGES.has(reqLanguage)) {
        console.warn(`[API] Invalid language "${reqLanguage}", defaulting to en-US`);
    }
    console.log(`[API] Convert-to-blocks request | Code length: ${code.length}`);
    try {
        const result = await llmService_1.llmService.convertToBlocks(code, language);
